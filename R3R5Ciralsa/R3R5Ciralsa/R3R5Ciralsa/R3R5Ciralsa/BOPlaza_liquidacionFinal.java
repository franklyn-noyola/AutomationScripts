...
public class BOPlaza_liquidacionFinal{	
	...
	public void liquidacionFinalInit() throws Exception{
		...
		if (bagDuplicate==true) {
			actualResults.set(5,"No se puede crear una Liquidación Final debido a: "+errorText);
			System.out.println("No se puede crear una Liquidación Final debido a: "+errorText);			
		}else {
			actualResults.set(5,"Se ha creado una liquidación final No. "+liquidacionNumbr+" con el No. de Turno: "+shiftNumbr+" y un Total en Bolsa: "+bagTotal);
			System.out.println("Se ha creado la liquidación final No. "+liquidacionNumbr+" con un total de bolsas "+bagNumbers+" con el No. de Turno: "+shiftNumbr+" y un Total en Bolsa/Liquidado: "+bagTotal+"/"+totalCashout);
		}				
		driver.close();
		Thread.sleep(1000);
		testLink();
	...
	public static void liquidacionFinal() throws Exception{
		...
		try{
			//Paso 1.- Entrar a la página de Login del BO Plaza de R3R5Ciralsa
			...
			Thread.sleep(100);
			SendKeys("ctl00_ContentZone_txt_comment_box_data", "Nueva Liquidación Final");
			Thread.sleep(100);			
			...
			if (driver.getPageSource().contains("valor introducido ya existe") || driver.getPageSource().contains("repetida en la liquidación")) {
